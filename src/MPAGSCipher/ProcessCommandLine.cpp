                        ...
                        ProgramSettings& settings)
{
        ...
        if (cmdLineArgs[i] == "-h" || cmdLineArgs[i] == "--help") {
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                for (const auto& elem : arg) {
                    if (!std::isdigit(elem)) {
                        throw InvalidArgument("--multi-cipher requires a positive integer argument and " + arg + " could not be converted to type int");
                    }
                }
            ...
            if (i == nCmdLineArgs - 1) {
                ...
                if (cmdLineArgs[i + 1] == "caesar") {
                    ...
                    throw InvalidArgument("Unknown cipher type " + cmdLineArgs[i+1] + " specified");
                }
                ++i;
            }
        } else {
            ...
            throw UnknownArgument("Command line argument " + cmdLineArgs[i] + " does not exist, try --help for options");
        }
    }
    ...
    if (nTypes != nExpectedCiphers || nKeys != nExpectedCiphers) {
        std::string str {"Expected types and keys for " + std::to_string(nExpectedCiphers) + " ciphers but recieved " + std::to_string(nTypes) + " types and " + std::to_string(nKeys) + " keys"};
        throw MissingArgument(str);
    }
    return true;
}